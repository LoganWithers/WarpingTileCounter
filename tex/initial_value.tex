...
Note that we use $bin(s[i])[j]$, where $i$ is the index of a digit in $\counterstart$ and $j$ is the index of a bit within the encoded digit.

\begin{itemize}
    \item Create
    $\begin{aligned}[t]
        {\tt Seed}(&\left\langle {\tt CounterWrite}, 1, {\tt seed}, 0, 0 \right\rangle \; )
    \end{aligned}$ \\ TODO: include special seed that has 4 additional tiles to stop fillers if $k$ is odd?
\end{itemize}

%
We repeat these steps starting from $i = 0$ and repeating until $i$ is the index of the first digit in the MSR.
%
These steps build general non-MSR digit regions shown in Figure~\ref{fig:initial_general}.
%
...
\begin{itemize}
    ...
    \item {\tt Digit}, for each $j=0,\ldots,l-1$, where $b = bin(s[i])[j]$:
    \begin{itemize}
        \item if $j = 0$:\\ create
        $\begin{aligned}[t]
...
\begin{align*}
    dl &= O \left( d \hspace{0.1cm} {\log m} \right) \\
       ...
       &= O \left( \hspace{0.1cm} {\log N} + \floor*{\frac{k}{2} } \right) \\
...
%
$167 + 28 \cdot \left( {\log m} + 3 \right) =$
%
...
%
$193 + 36 \cdot \left( {\log m} + 3 \right) =$
%
$O \left( {\log m} \right) $ tiles were created in this step.
%
...
%
$304 + 48 \cdot \left( {\log m} + 3 \right) =$
%
$O \left( {\log m} \right) $ tiles were created in this step.
%
